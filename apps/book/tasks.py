...
def upload_files_to_firebase(self, file_field, file_type, isbn, folder='audiobooks', subfolder=None):
    try:
        ...
        messages = []
        if book.poster_url:
            messages.append(f"Uploaded {file_type} for ISBN {isbn}: {book.poster_url}")
        if book.cover_url:
            messages.append(f"Uploaded {file_type} for ISBN {isbn}: {book.cover_url}")
        if book.ebook_url:
            messages.append(f"Uploaded {file_type} for ISBN {isbn}: {book.ebook_url}")
        return messages
